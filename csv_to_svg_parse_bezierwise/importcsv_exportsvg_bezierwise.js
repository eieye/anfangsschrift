...
// 16-JUL-23
// EXPORTING BEZIER (AS SEPARATE SEGMENTS) IN SVG-FORMAT
...
// GLOBAL SCOPE
  ...
  let csv_import_raw;
	...
	let pathD = "";																																			// THE SVG PATH-TAG "D"-ARG FOR BEZIER-TUPLE
...
// ADD COMMENT INSIDE SVG-GROUP
	let newstroke = false;
	const COMMENT = "<!-- STROKE -->";
	var svg_composite;  											 											 										// THE EXPORT STRING
...
// ----------------------------------------------------------------------------------------------
							// PROCESS LONG PATH_FORMAT (REPEAT "M") FOR PATHWISE BEZIER IMPLEMENTATION

	for (let j = 0; j < strokes.length - 1; j++) {																			// SKIP LAST (UNDEFINED) ELEMENT OF "STROKES"
		stroke = strokes[j];
		newstroke = true; // ADD COMMENT
...
// ADD REDUNDANT (REPEATED) ANCHOR-PT (A1) TO TUPLE FOR EACH SEGMENT
		for (let i = 0; i < stroke.length; i += 4) {

			ax = stroke[i][0];							// A1
			ay = stroke[i][1] - SHIFTVAL;
			bx = stroke[i+1][0];						// C1
...
`; // (NEWLINE)
			...
			segment += `  <path d="M ${ax} ${ay} C ${bx} ${by} ${cx} ${cy} ${dx} ${dy}" />
`; // (NEWLINE)
		...
		svg_composite = concatSEGMENTtoSVG(pathD);
	} // FOR STROKE
...
function concatSEGMENTtoSVG(pathD) {
